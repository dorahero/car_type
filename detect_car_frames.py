...
#         modelc.to(device).eval()
    ...
    count_json = 1
    count = 0
    gps_col = ["gps_time", "lat", "lon", "alt(m)", "speed(km/h)", "climb(m/s)", "track", "eps", "epx", "epv", "ept", "fixtype"]
    json_all = []
    for path, img, im0s, vid_cap in dataset:
#         print(img.shape)
#         print(im0s.shape)
        json_data = {"iot_id": '', "timestamp": '', "gps_time": '',"lat": '', "lon": '', "alt(m)": '',
                     "speed(km/h)": '', "climb(m/s)": '', "track": '', "eps": '', "epx": '', "epv": '',
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if car_count == 0:
                        car_name = 'car_o.' + str(timestamp) + '.jpg'
                        cv2.imwrite('/home/jn/hunter_data/{}/{}/frames/{}'.format(opt.iot_id, opt.dir, car_name), im0)
                        car_count += 1
                    # car ratio
...
#                     print('im0_0', im0.shape)
                    if ratio < 0.2:
                        ...
                        xyxy_lp, label_lp, label_num, conf_num = detect_lp(img0=target_img, opt=opt, model=model_1, model_2=model_2, model_3=model_3, half=half, webcam=webcam, device=device, far=False)
...
#                     vid_writer.write(im0)
        # save frames
        jpg_name = 'car.' + str(timestamp) + '.jpg'
        # if len(json_data["cars"]):
        #     cv2.imwrite('/home/jn/hunter_data/{}/{}/frames/{}'.format(opt.iot_id, opt.dir, jpg_name), im0)
        cv2.imwrite('/home/jn/hunter_data/{}/{}/frames/{}'.format(opt.iot_id, opt.dir, jpg_name), im0)
        # jpg_name = 'id.jpg'.format(timestamp)
        json_data["jpg_path"] = jpg_name
        # cv2.imwrite('../frames/{}'.format(jpg_name), im0)
        print('json=', json_data)
        json_all.append(json_data)
        # save json
        if count > 800:
            json_str = json.dumps(json_all)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--iot-id', default='n1')
